...
#ifdef IBPP_WINDOWS
// New (optional) Registry Keys introduced by Firebird Server 1.5 - для 2,5 ключ не изменился
#define REG_KEY_ROOT_INSTANCES (LPCWSTR)	"SOFTWARE\\Firebird Project\\Firebird Server\\Instances"
#define FB_DEFAULT_INSTANCE	  	(LPCWSTR) "DefaultInstance"
#endif
...
#undef max
	...
	std::ostream& operator<< (std::ostream& a, flush_debug_stream_type)
	{
		...
		{
#ifdef IBPP_WINDOWS
			::OutputDebugString((LPCWSTR) ("IBPP: " + p->str() + "\n").c_str());
//non unicode:        ::OutputDebugString( ("IBPP: " + p->str() + "\n").c_str());
#endif
			p->str("");
		}
		return a;
	...
	if (! mReady)
	{
		...
		while (pos < mSearchPaths.size())
		{
			...
			if (path.size() >= 1)
			{
				...
				mHandle = LoadLibrary((LPCWSTR) path.c_str());
		...
		if (mHandle == 0)
		{
			...
			int len = GetModuleFileName(NULL, (LPWCH)fbdll, sizeof(fbdll));
//			int len = GetModuleFileName(NULL, fbdll, sizeof(fbdll));
			...
			if (len != 0)
			{
				...
				do {--p;} while (*p != '\\');
				*p = '\0';
				lstrcat((LPWSTR) fbdll, (LPWSTR)"\\fbembed.dll");// Local copy could be named fbembed.dll
//				lstrcat(         fbdll, "\\fbembed.dll");// Local copy could be named fbembed.dll
				mHandle = LoadLibrary((LPCWSTR) fbdll);
//                mHandle = LoadLibrary(fbdll);
				if (mHandle == 0)
				{
					*p = '\0';
                    lstrcat((LPWSTR)fbdll, (LPWSTR)"\\fbclient.dll");	// Or possibly renamed fbclient.dll
...
//					mHandle = LoadLibrary(fbdll);
		...
		if (mHandle == 0)
		{
			...
			if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, REG_KEY_ROOT_INSTANCES, 0,	KEY_READ, &hkey_instances) == ERROR_SUCCESS)
			{
				DWORD keytype;
				DWORD buflen = sizeof(fbdll);
				if (RegQueryValueEx(hkey_instances, (LPCWSTR) "DefaultInstance", 0,
						&keytype, reinterpret_cast<UCHAR*>( fbdll), &buflen) == ERROR_SUCCESS && keytype == REG_SZ)
				{
					lstrcat((LPWSTR)fbdll,(LPWSTR) "bin\\fbclient.dll");
					mHandle = LoadLibrary((LPCWSTR)fbdll);
				}
		...
		// Проблемы с unicode, вместо  LoadLibrary((LPCWSTR) напишем вот так:
